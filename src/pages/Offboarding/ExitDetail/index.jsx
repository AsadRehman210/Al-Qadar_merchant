...
const ExitDetail = () => {
  ...
  const department = emp ? departmentsById[emp.departmentId]?.name || "—" : "—";
  const designation = emp ? designationsById[emp.designationId]?.title || "—" : "—";
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-white/10 border border-slate-200 dark:border-white/20 rounded-3xl p-7 border-l-4 !border-l-[var(--color-teal-500)]">
              ...
              <div className="grid grid-cols-2 gap-y-3 text-sm">
                ...
                <span className="font-medium text-slate-800 dark:text-white">{exit.resignationDate ? dayjs(exit.resignationDate).format("YYYY-MM-DD") : "—"}</span>
                <span className="text-slate-500 dark:text-white/50">{t("offboarding:last_working_day")}</span>
                <span className="font-medium text-slate-800 dark:text-white">{exit.lastWorkingDay ? dayjs(exit.lastWorkingDay).format("YYYY-MM-DD") : "—"}</span>

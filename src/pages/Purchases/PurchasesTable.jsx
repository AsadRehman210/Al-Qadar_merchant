...
const PurchasesTable = ({ data, loading, page = 1, setPage, totalRecords = 0, perPage = 10, onDeleteConfirmed }) => {
          ...
          <table className="w-full border-collapse text-sm mb-0">
            <thead>
              <tr className="bg-[var(--color-teal-500)] border-none">
                ...
                <th className="w-[180px] px-4 py-4 text-start font-semibold text-white/95 border-none whitespace-nowrap pr-6 rounded-tr-2xl">
                  {t("customers:actions")}
                </th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={10} className="text-center py-10 text-slate-400">{t("loading")}</td></tr>
              ) : list.length === 0 ? (
                <tr><td colSpan={10} className="text-center py-10 text-slate-400">{t("no_record_found")}</td></tr>
              ) : list.map((row) => (
                <tr
                  key={row.id}

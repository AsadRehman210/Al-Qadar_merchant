...
const QuickActionPanel = ({ leave, onDone }) => {
  ...
  const submit = async () => {
    try {
      if (action === "approve") {
        await dispatch(managerApproveLeave({ id: leave.id, comments })).unwrap();
      } else {
        await dispatch(managerRejectLeave({ id: leave.id, comments })).unwrap();
      }
      onDone?.();
      setOpen(false);
    } catch (err) {
      ...
      <div className="mt-3 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30">
        ...
        <div className="flex gap-2">
          <Button
            ...
            btn="primary"
            className={`!w-auto !rounded-lg !h-8 !px-3 !text-xs !border-0 !text-white ${action === "approve" ? "!bg-emerald-500 hover:!bg-emerald-600" : "!bg-rose-500 hover:!bg-rose-600"}`}
            onClick={submit}
          />
          <Button
            type="button"
...
const ManagerApprovals = () => {
  ...
  const [filters, setFilters] = useListFilters("hr-leave-manager-approvals", { page: 1, limitId: cardRows[0].id });
  const selRows = cardRows.find((r) => r.id === filters.limitId) || cardRows[0];
  const page = filters.page;

  useEffect(() => {
    dispatch(fetchLeaves({ status: "Pending Manager", page, limit: selRows.id }));
  ...
  const handleDone = () => dispatch(fetchLeaves({ status: "Pending Manager", page, limit: selRows.id }));
    ...
    <div className="relative min-h-[60vh] overflow-hidden">
      ...
      <div className="relative z-[1] space-y-6">
        <div className="flex flex-wrap items-center gap-4 dark:text-white">
          ...
          />
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{t("leave:manager_approvals")}</h1>
            <p className="text-mutedForeground">{t("leave:manager_approvals_desc")}</p>
          </div>
          <span className="ltr:ml-auto rtl:mr-auto text-2xl font-bold text-amber-600">
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {pending.map((leave) => (
                <div
                  key={leave.id}
                  className="bg-white dark:bg-white/10 border-2 border-amber-200 dark:border-amber-500/30 rounded-3xl p-6"
                >
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-2xl text-amber-600">{leave.days}</p>
                    </div>
                  </div>
                  {leave.reason && (

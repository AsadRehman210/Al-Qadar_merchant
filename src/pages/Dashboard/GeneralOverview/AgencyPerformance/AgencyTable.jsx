...
const AgencyTable = () => {
  ...
  return (
    <Card className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 hover:shadow-lg mt-8">
        ...
        <table className="w-full mb-5 min-w-[800px]">
          ...
          <thead>
            <tr className="[&_th]:border-b [&_th]:font-medium [&_th]:text-[#64748b] [&_th]:border-gray-300 [&_th]:px-4 [&_th]:py-2 [&_th]:text-start">
              ...
              <th>{t("dashboard:pct_total_booking")}</th>
            </tr>
          </thead>

          {/* Table Body */}
          <tbody>
            <TableState loading={status} data={revenueByAgency} colSpan={8}>
              {revenueByAgency?.map((item, idx) => {
                return (
                  <tr
                    key={idx}
                  ...
                  >
                    ...
                    <td className="font-medium text-sm text-linkText whitespace-nowrap">
                      {item?.percentage_of_total_bookings}
                    </td>
                  </tr>
                );
              })}

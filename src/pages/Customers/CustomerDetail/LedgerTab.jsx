...
const LedgerTab = ({ customer }) => {
  ...
  useEffect(() => {
    if (customer?.id) dispatch(fetchCustomerLedger({ id: customer.id, page, limit: LIMIT }));
  }, [dispatch, customer?.id, page]);
          ...
          <thead>
            <tr className="bg-slate-100 dark:bg-white/10">
              <th className="px-4 py-3 text-start font-semibold text-slate-900 dark:text-white">
                {t("customers:ledger_date")}
              </th>
              <th className="px-4 py-3 text-start font-semibold text-slate-900 dark:text-white">
                {t("customers:ledger_type")}
              </th>
              <th className="px-4 py-3 text-start font-semibold text-slate-900 dark:text-white">
                {t("customers:ledger_reference")}
              </th>
              <th className="px-4 py-3 text-start font-semibold text-slate-900 dark:text-white">
                {t("customers:ledger_debit")}
              </th>
              <th className="px-4 py-3 text-start font-semibold text-slate-900 dark:text-white">
                {t("customers:ledger_credit")}
              </th>
              <th className="px-4 py-3 text-start font-semibold text-slate-900 dark:text-white">
                {t("customers:ledger_running_balance")}
              </th>
            </tr>
          </thead>
          <tbody>

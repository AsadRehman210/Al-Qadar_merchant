...
const CustomersTable = ({ data, loading, page = 1, setPage, selRows, setSelRows, totalPages, onDeleted }) => {
  ...
  const onConfirmDelete = async (row) => {
    try {
      const deleted = await dispatch(deleteSalesCustomer(row.id)).unwrap();
      await onDeleted?.();
  ...
  return (
    <>
              ...
              <TableState loading={loading} data={list} colSpan={9}>
                {list.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 align-middle text-slate-600 dark:text-white/90">{row.customerType || "-"}</td>
      ...
      <ActionPopup
        ...
        description={t("customers:confirm_delete_customer")}
        confirm={t("yes")}
        cancel={t("cancel")}

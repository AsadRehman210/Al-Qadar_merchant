...
import {
  fetchCollectedTaxReport,
  showCollectedTax,
  showCollectedTaxTotal,
  showCollectedTaxTotalAmount,
  showCollectedTaxLoading,
...
const CollectedTax = () => {
  ...
  const rows = useSelector(showCollectedTax);
  const total = useSelector(showCollectedTaxTotal);
  const totalAmount = useSelector(showCollectedTaxTotalAmount);
  const loading = useSelector(showCollectedTaxLoading);
  const [filters, setFilters] = useListFilters("finance-collected-tax", { search: "", page: 1, limitId: tableRows[0].id });
  const { search, page } = filters;
  const selRows = tableRows.find((r) => r.id === filters.limitId) || tableRows[0];

  useEffect(() => {
    dispatch(fetchCollectedTaxReport({ page, limit: selRows.id, search: search || undefined }));
  ...
  return (
    <FinancePage title={t("finance:collected_tax_title")} description={t("finance:collected_tax_desc")}>
            ...
            <table className="w-full border-collapse text-sm mb-0 min-w-[900px]">
              <thead>
                <tr className="bg-[var(--color-teal-500)] border-none">
                  <th className="px-4 py-4 text-start font-semibold text-white/95 border-none whitespace-nowrap pl-6 rounded-tl-md">{t("sales:invoice_number")}</th>
                  <th className="px-4 py-4 text-start font-semibold text-white/95 border-none whitespace-nowrap">{t("sales:customer")}</th>
                  <th className="px-4 py-4 text-start font-semibold text-white/95 border-none whitespace-nowrap">{t("sales:date")}</th>
                  <th className="px-4 py-4 text-end font-semibold text-white/95 border-none whitespace-nowrap">{t("sales:subtotal")}</th>
                  <th className="px-4 py-4 text-end font-semibold text-white/95 border-none whitespace-nowrap pr-6 rounded-tr-md">{t("sales:tax_amount")}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  {rows.map((row) => (
                    <tr key={row.id} className="transition-colors border-b border-slate-100 dark:border-white/5 hover:bg-teal-50 dark:hover:bg-teal-500/10 last:[&_td]:border-b-0">
                      <td className="px-4 py-4 align-middle pl-6 font-mono text-xs">
                        <Link to={`/sales/detail/${row.id}`} className="text-teal-700 hover:underline dark:text-teal-400">{row.invoiceNumber}</Link>
                      </td>

...
// permanent ledger the moment it's created (same rule Journal Entries
// follow). Distinct from the HR Employee Expenses module.
const AddExpense = () => {
  ...
  const expenseOpts = useMemo(
    () => coaAccounts.filter((a) => a.type === "Expense").map((a) => ({ id: a.id, title: `${a.code} — ${a.name}` })),
    [coaAccounts],
  );
  ...
  const { register, handleSubmit } = useForm({
    defaultValues: { date: new Date().toISOString().slice(0, 10), category: "", description: "", amount: "" },
  });
      ...
      <div className="relative z-[1]">
        <div className="mb-7 flex flex-wrap items-center gap-4 dark:text-white">
          <Button
            ...
            iconClass="!text-lg"
          />
          <h1 className="text-3xl font-bold tracking-tight">{t("finance:add_expense")}</h1>
        </div>
        <form
        ...
        >
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <FormInput label={t("finance:category")} name="category" maxLength={200} register={register} required />
            ...
            <div>
              <label className="text-sm font-medium mb-1 block">{t("finance:category")}</label>
              <SelectDropdown data={expenseOpts} selected={selExpense} setSelected={setSelExpense} hideClear classes="!h-[46px] !rounded-lg" />
            </div>
          </div>
          <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-slate-200 dark:border-white/20">
            <Button type="button" title={t("cancel")} onClick={() => navigate("/finance/expenses")} />

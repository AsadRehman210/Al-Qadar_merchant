...
const BillDetail = () => {
  ...
  const onApprove = async () => {
    setBusy(true);
    try {
      const result = await dispatch(approveVendorBill(id));
      if (result.error) throw new Error(result.payload || t("finance:save_failed"));
      toast.success(t("finance:bill_approved"));
    } catch (err) {
  ...
  const onCancel = async () => {
    setBusy(true);
    try {
      const result = await dispatch(cancelVendorBill(id));
      if (result.error) throw new Error(result.payload || t("finance:save_failed"));
      toast.success(t("finance:bill_cancelled"));
    } catch (err) {
      toast.error(err.message);
    } finally {
      setBusy(false);
      ...
      <div className="relative z-[1]">
        <div className="mb-7 flex flex-wrap items-center justify-between gap-4 dark:text-white">
          <div className="flex items-center gap-4">
            ...
            />
            <div>
              <h1 className="text-2xl font-bold tracking-tight">{bill.vendorName}</h1>
              <p className="text-sm text-mutedForeground font-mono">{bill.billNumber}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            {bill.status === "Draft" && checkRoleAuth(edit_customer) && (
              <>
                ...
                <Button type="button" title={t("finance:cancel_bill")} onClick={onCancel} disabled={busy} className="!bg-rose-50 !text-rose-700 dark:!bg-rose-500/20 dark:!text-rose-300" />
                <Button type="button" title={t("finance:approve_bill")} onClick={onApprove} disabled={busy} btn="primary" />
              </>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-white/10 border border-slate-200 dark:border-white/20 rounded-3xl p-8 border-l-4 !border-l-[var(--color-teal-500)]">
          <div className="grid sm:grid-cols-3 gap-4 mb-6">
            <div>
              <p className="text-xs text-mutedForeground">{t("finance:posted_date")}</p>
          ...
          <table className="w-full text-sm mb-6">
            <thead>
              <tr className="text-left text-xs text-mutedForeground border-b border-slate-200 dark:border-white/10">
                ...
                <th className="pb-2">{t("finance:category")}</th>

...
import {
  ...
  showPayables,
  showPayablesTotal,
  showPayablesTotalBalanceDue,
  showPayablesTotalRefundDue,
  showPayablesLoading,
...
// The real Accounts Payable view — every Purchase Invoice this tenant
// actually still owes on (or is owed a refund back from), sourced live from
// the real Purchases module (balanceDue/refundDue, already computed
// server-side factoring in Debit Notes) rather than a separate manually
// entered "Vendor Bill" record that never overlaps with real purchases.
const Payable = () => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const rows = useSelector(showPayables);
  const total = useSelector(showPayablesTotal);
  const totalBalanceDue = useSelector(showPayablesTotalBalanceDue);
  const totalRefundDue = useSelector(showPayablesTotalRefundDue);
  const loading = useSelector(showPayablesLoading);
  const [filters, setFilters] = useListFilters("finance-payable", { search: "", page: 1, limitId: tableRows[0].id });
  const { search, page } = filters;
  const selRows = tableRows.find((r) => r.id === filters.limitId) || tableRows[0];

  useEffect(() => {
    dispatch(fetchPayables({ page, limit: selRows.id, search: search || undefined }));
  }, [dispatch, page, selRows.id, search]);

  const totalPages = Math.max(1, Math.ceil((total || 0) / selRows.id));

  return (
    <FinancePage title={t("finance:ap_title")} description={t("finance:ap_desc")}>
      {checkRoleAuth(view_finance_payable) && (
        <>
          {loading ? (
            ...
            <div className="mb-6 grid grid-cols-2 sm:grid-cols-3 gap-4 max-w-2xl">
              <div className="p-4 rounded-2xl border border-rose-200 bg-rose-50 dark:bg-rose-500/10 dark:border-rose-500/20">
                <p className="text-xs text-slate-500">{t("finance:total_payable", { defaultValue: "Total payable" })}</p>
              ...
              <div className="p-4 rounded-2xl border border-amber-200 bg-amber-50 dark:bg-amber-500/10 dark:border-amber-500/20">
                <p className="text-xs text-slate-500">{t("finance:total_refund_due", { defaultValue: "Refund due from suppliers" })}</p>
            ...
            <table className="w-full border-collapse text-sm mb-0 min-w-[900px]">
              <thead>
                <tr className="bg-[var(--color-teal-500)] border-none">
                  <th className="px-4 py-4 text-start font-semibold text-white/95 border-none whitespace-nowrap pl-6 rounded-tl-md">{t("finance:supplier")}</th>
                    ...
                    return (
                      <tr key={row.id} className="transition-colors border-b border-slate-100 dark:border-white/5 hover:bg-teal-50 dark:hover:bg-teal-500/10 last:[&_td]:border-b-0">
                        <td className="px-4 py-4 align-middle pl-6 font-medium">
                          <Link to={`/suppliers/detail/${row.supplierId}`} className="text-teal-700 hover:underline dark:text-teal-400">
                            {row.supplierName}
                          </Link>
                        </td>

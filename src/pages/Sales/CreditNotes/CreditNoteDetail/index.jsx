...
import {
  ...
  showCurrentCreditNote,
  showCurrentCreditNoteLoading,
...
const CreditNoteDetail = () => {
  ...
  // on at least one line — same detection rule Sale Detail's own table uses.
  const hasDifferentTax = (cn.products || []).some((l) => l.taxPercent !== null && l.taxPercent !== undefined);
      ...
      <div className="relative z-[1]">
          ...
          <div className="mb-5 p-4 rounded-2xl border border-blue-200 bg-blue-50 dark:bg-blue-500/10 flex flex-wrap items-end gap-3">
            <div>
              ...
              <select value={newStatus} onChange={(e) => setNewStatus(e.target.value)}
                ...
                {(NEXT_STATUS[cn.status] || []).map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: t("sales:discount"), value: `${cn.currency} ${fmt(cn.discount)}`,  color: "border-rose-200 bg-rose-50 dark:bg-rose-500/10" },
            ...
            <div>
              ...
              <p className={`font-semibold mt-0.5 ${RESTOCK_REASONS.has(cn.reason) ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}>
                {RESTOCK_REASONS.has(cn.reason)
                  ? t("sales:restock_yes", { defaultValue: "Restocked on Apply" })
                  : t("sales:restock_no", { defaultValue: "Not restocked (write-off)" })}
              </p>
            </div>
          </div>
              ...
              <tbody>
                {(cn.products || []).map((l, idx) => (
                  <tr key={l.id || l.variantId || idx} className="border-t border-slate-100 dark:border-white/5">
                    ...
                    <td className="px-4 py-2.5 font-semibold tabular-nums text-rose-600">{fmt(lineTotal(l) + (l.taxAmount || 0))}</td>
                  </tr>
                ))}
              </tbody>
            </table>

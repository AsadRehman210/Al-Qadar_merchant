...
const CreditNotes = () => {
  ...
  const rows = useSelector(showCreditNotes);
  const total = useSelector(showCreditNotesTotal);
  const loading = useSelector(showCreditNotesLoading);

  useEffect(() => {
    dispatch(fetchCreditNotes({ page, limit: selRows.id, search: search || undefined }));
              ...
              <tbody>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer"
                    ...
                    <td className="px-4 py-3 font-mono text-teal-600">{r.cnNumber}</td>

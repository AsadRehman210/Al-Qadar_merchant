    ...
    <div className={panelClass}>
      ...
      {invoice.notes ? (
        ...
        <p className="text-slate-500 dark:text-white/60 text-center py-6 mb-8">
          {t("sales:no_notes_available", { defaultValue: "No notes available" })}
        </p>
      )}
      <ul className="space-y-4">
        {(invoice.notesLog || []).map((n) => (
          <li
            key={n.id}
            className="rounded-md border border-slate-200 dark:border-white/20 p-4 bg-slate-50/80 dark:bg-white/5"
          >

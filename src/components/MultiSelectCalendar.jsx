...
export default function MultiSelectDropdown({
  ...
  const [pendingId, setPendingId] = useState(null); // 👈 track selected option in progress
  ...
  const handleOptionClick = (person) => {
    ...
    const isSelected = selected.some((s) => s._id === person._id);
    const newSelection = isSelected
      ? selected.filter((s) => s._id !== person._id)
      : [...selected, person];

    // 🔥 Immediately show loader

    if (onSelectWithLoading) {
      onSelectWithLoading(person)
        .then(() => setSelected(newSelection))
        .finally(() => setPendingId(null)); // hide loader after API
    } else {
        ...
        >
          <ComboboxOptions
            ...
            className={`w-[var(--button-width)] ${
              listClass || ""
            } max-h-[250px] z-[9999] rounded-lg border shadow-lg bg-white p-1 empty:hidden`}
            static
          >
                  ...
                  <ComboboxOption
                    ...
                    className={`${
                      ...
                      selected?.some((item) => item?._id === person?._id)
                        ? "bg-black/10"
                        : ""
                    } group flex hover:bg-gray-100 cursor-pointer items-center gap-2 rounded-lg py-1.5 my-px px-3 select-none justify-between`}
                    onClick={() => handleOptionClick(person)}
                  >
                    <span>{person.title}</span>
                    {/* Loader takes priority */}
                    {!calendarLoading && pendingId === person._id ? (
                      <div className="animate-spin h-4 w-4 border-b-2 border-gray-900 rounded-full ml-2" />
                    ) : isSelected ? (
